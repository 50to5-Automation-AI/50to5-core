...
ENGINEERING_SCHEMA = {
    ...
    "required": ["task_id", "summary", "details", "priority", "status"]
}
...
MARKETING_SCHEMA = {
    ...
    "required": ["campaign_id", "title", "description", "deadline", "status"]
}
...
class GroupManagerAgent(GroupChatManager):
    ...
    def validate_json(self, data: Dict[str, Any], schema: Dict[str, Any]):
        try:
            validate(instance=data, schema=schema)
        except jsonschema.exceptions.ValidationError as err:
            raise ValueError(f"Invalid data: {err.message}")

    def create_task_in_jira(self, task_details: Dict[str, Any]) -> str:
        self.validate_json(task_details, ENGINEERING_SCHEMA)
    ...
    def create_task_in_clickup(self, task_details: Dict[str, Any]) -> str:
        self.validate_json(task_details, MARKETING_SCHEMA)
        response = createAgentTask({
